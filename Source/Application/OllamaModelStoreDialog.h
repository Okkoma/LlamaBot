...
public:
    ...
    // C++ version
    void fetchManifest(const QString &modelName, std::function<void(bool success, const ModelManifest &manifest, const QString &error)> callback);
    ...
    Q_INVOKABLE void fetchLibraryQml() {
        fetchLibrary([](bool, const QVector<LibraryModel>&, const QString&){});
    };

    // Download a specific layer (blob) to a destination path
...
private:
    ...
    void handleManifestResponse(QNetworkReply *reply, std::function<void(bool, const ModelManifest &, const QString &)> callback);
};
