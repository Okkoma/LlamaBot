...
OllamaModelStoreDialog::OllamaModelStoreDialog(QObject *parent)
    : QObject(parent), m_manager(new QNetworkAccessManager(this)),
      m_currentDownloadReply(nullptr), m_outputFile(nullptr) {}

OllamaModelStoreDialog::~OllamaModelStoreDialog() { cancelDownload(); }

QString OllamaModelStoreDialog::parseModelName(const QString &input,
                                               QString &tag) {
  QString name = input;
  tag = "latest";
  if (name.contains(':')) {
...
void OllamaModelStoreDialog::fetchManifest(
    ...
    std::function<void(bool, const ModelManifest &, const QString &)>
        callback) {
  QString tag;
  QString cleanName = parseModelName(modelName, tag);

  QUrl url(QString("https://registry.ollama.com/v2/%1/manifests/%2")
               .arg(cleanName, tag));
...
void OllamaModelStoreDialog::fetchManifest(const QString &modelName,
                                           QJSValue callback) {
  // Capture callback by value (copying QJSValue is cheap/reference-like)
  QJSValue cb = callback; // Mutable copy

  fetchManifest(modelName, [this, cb](bool success,
...
void OllamaModelStoreDialog::fetchLibrary(
    std::function<void(bool, const QVector<LibraryModel> &, const QString &)>
        callback) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply, callback]() {
    ...
    // 1. Inject Essentials/Popular Models (since api/tags is
    // incomplete/recent-only)
    struct Featured {
      const char *name;
      const char *desc;
    };
    std::vector<Featured> featured = {
        {"llama3.2", "Meta's latest efficient model"},
        {"llama3.1", "Meta's powerful open model"},
        {"mistral", "Mistral AI 7B model"},
        {"gemma2", "Google's open model"},
        {"phi3.5", "Microsoft's small model"},
        {"llava", "Multi-modal (Vision) model"},
    ...
    for (const QJsonValue &val : modelsArray) {
      ...
      m.name =
          modelObj["model"]
              .toString(); // In api/tags, 'model' and 'name' seem identical
      m.tag = m.name;
...
void OllamaModelStoreDialog::downloadLayer(const QString &modelName,
  ...
  QUrl url(QString("https://registry.ollama.com/v2/%1/blobs/%2")
               .arg(cleanName, digest));
  QNetworkRequest request(url);
  request.setAttribute(QNetworkRequest::RedirectPolicyAttribute,
                       QNetworkRequest::NoLessSafeRedirectPolicy);
  ...
  connect(m_currentDownloadReply, &QNetworkReply::downloadProgress, this,
          &OllamaModelStoreDialog::downloadProgress);

  connect(m_currentDownloadReply, &QNetworkReply::readyRead, this, [this]() {
          ...
          [this, savePath]() {
            ...
            if (m_currentDownloadReply->error() == QNetworkReply::NoError) {
              ...
              emit downloadFinished(false,
                                    m_currentDownloadReply->errorString());
            }
