...
void OllamaModelStoreDialog::fetchManifest(const QString &modelName, std::function<void(bool, const ModelManifest &, const QString &)> callback)
{
    QString tag;
    ...
    request.setRawHeader("Accept", "application/vnd.docker.distribution.manifest.v2+json, application/vnd.oci.image.manifest.v1+json");
...
void OllamaModelStoreDialog::handleManifestResponse(QNetworkReply *reply, std::function<void(bool, const ModelManifest &, const QString &)> callback)
{
...
void OllamaModelStoreDialog::fetchManifest(const QString &modelName, QJSValue callback)
{
    ...
    fetchManifest(modelName, [this, cb](bool success, const ModelManifest &manifest, const QString &error) mutable {
        if (!cb.isCallable()) return;
        
        // Use the injected engine or try to get one
        QJSEngine *engine = m_qmlEngine;
        if (!engine) {
            qWarning() << "OllamaModelStoreDialog: No QJSEngine set, cannot convert objects for callback!";
...
void OllamaModelStoreDialog::fetchLibrary(std::function<void(bool, const QVector<LibraryModel> &, const QString &)> callback)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply, callback]() {
        ...
        for (const QJsonValue &val : modelsArray) {
            ...
            // FILTER: Skip "Cloud" or Massive models (> 100 GB) or Invalid/Placeholder (<= 0)
            ...
            m.description = QString("%1 (%2)").arg(m.name).arg(modelObj["modified_at"].toString().left(10)); // Generate simple desc
...
void OllamaModelStoreDialog::downloadLayer(const QString &modelName, const QString &digest, const QString &savePath)
{
    ...
    connect(m_currentDownloadReply, &QNetworkReply::finished, this, [this, savePath]() {
        if (m_outputFile) {
            m_outputFile->close();
            delete m_outputFile;
            m_outputFile = nullptr;
